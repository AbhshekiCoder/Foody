  ...
  return (
    ...
    {dish?dish.map((Element)  =>(
        ...
        <div className='p-3 '>
            ...
            <div className='absolute   flex justify-center ml-2 mt-36 max-md:mt-32 max-sm:mt-20 max-sm:ml-0 border' >
            {localStorage.getItem(Element._id)?
            ...
            <div className='btn   w-36 h-9 bg-white font-bold text-lg max-md:w-32 max-sm:text-md max-sm:w-24   justify-between hidden items-center shadow-md absolute' id ={Element._id}><button className='text-green-400 text-2xl' onClick={() =>add(Element.name, Element.id, Element._id)}>+</button><p className="text-gray-500" itemId={Element._id}>{localStorage.getItem(`${Element._id}`?localStorage.getItem(`${Element._id}`):0)}</p><button className='text-green-400 text-2xl' onClick={() =>added(Element.name, Element.restaurant_name, Element._id, Element.price)}>-</button></div>   
           </div>
