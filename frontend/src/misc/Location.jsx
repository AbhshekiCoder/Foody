...
export default function Location() {
  const api = "AIzaSyB3Et0gdbu18Id43ibuYkD1Ggd3hVHkono"; // Keep this private in env
  ...
  const getAddressFromLatLng = async (lat, lng) => {
    ...
    try {
      ...
      if (data) {
        ...
        await axios.post(`${url}profileUpdated/profileUpdated`, obj, {
          headers: {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setSelectPosition(currentLocation);
          setDropdownOption([currentLocation]);
          getAddressFromLatLng(latitude, longitude);
        },
        () => {
          console.log("Location access denied");
          // Default location fallback
          setSelectPosition({ lat: 20.5937, lng: 78.9629 }); // India
        }
      );
    } else {
      setSelectPosition({ lat: 20.5937, lng: 78.9629 }); // fallback
    }
  }, []);
  ...
  return (
    <div className='location w-full absolute hidden z-10 bg-white'>
      {/* Close button */}
      <div className='flex justify-end p-3'>
        <i className="fa-solid fa-xmark text-gray-500 text-xl cursor-pointer" onClick={() => {
      ...
      <LoadScript googleMapsApiKey={api}>
        <div className='px-4 pb-4'>
          ...
          <GoogleMap
            ...
            center={selectPosition || { lat: 20.5937, lng: 78.9629 }}
            zoom={13}
